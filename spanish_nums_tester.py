    ...
    if (numeral==0): # exception: 0 is "cero"
        ...
        if (dig_one_to_three==1): # one million
            ...
            if (answer[len(answer)-3:] == "uno"):
                if (answer[len(answer)-4:] == " uno"): # ex. 131000000 = ciento treinta y un millones (not uno)
                    answer=answer[0:len(answer)-1]
                else:
                    answer=answer[0:len(answer)-3]+"ún" # ex. 121000000 = ciento veintiún millones (not veintiuno)
            answer+=" millones "
        if (dig_four_to_six>1): # add thousands digits to string if applicable
            answer+= three_dig_num(dig_four_to_six)
            if (answer[len(answer)-3:] == "uno"):
                if (answer[len(answer)-4:] == " uno"): # ex. 131000 = ciento treinta y un mil (not uno)
                    answer=answer[0:len(answer)-1]
                else:
                    answer=answer[0:len(answer)-3]+"ún" # ex. 121000 = ciento veintiún mil (not veintiuno)
